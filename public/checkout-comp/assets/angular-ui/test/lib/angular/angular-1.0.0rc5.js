    ...
    return function module(name, requires, configFn) {
      ...
      return ensure(modules, name, function() {
        ...
        var moduleInstance = {
           ...
           * @methodOf angular.Module
           * @param {string} name Controllers name.
...
function $RouteProvider(){
   ...
   *    Object properties:
   *
   *    - `controller` â€“ `{function()=}` â€“ Controllers fn that should be associated with newly
   *      created scope.
...
function textInputType(scope, element, attr, ctrl, $sniffer, $browser) {
  ...
  if ($sniffer.hasEvent('input')) {
    ...
    // if users paste into input using mouse, we need "change" event to catch it
    ...
    link: function(scope, element, attr, ctrl) {
      ...
      function Options(scope, selectElement, ctrl) {
        ...
        var displayFn = $parse(match[2] || match[1]),
            ...
            nullOption = false, // if false then users will not be able to select it
        ...
        forEach(selectElement.children(), function(option) {
          if (option.value == '') {
            // developer declared null option, so users should be able to select it
