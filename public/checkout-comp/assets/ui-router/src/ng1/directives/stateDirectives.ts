 ...
 * The state's `userId` parameter value comes from each users's `users.id` property.
 * ```html
 * <li ng-repeat="users in users">
 *   <a ui-sref="users.detail({ userId: users.id })">{{ users.displayName }}</a>
 * </li>
 * ```
 *
 * Note:
 ...
 * #### Example:
 * ```html
 * <li ng-repeat="users in users" ui-sref-active="active">
 *   <a ui-sref="users.details({ userId: users.id })">{{ users.lastName }}</a>
 * </li>
 * ```
 *
 * ### Examples
 ...
 * ```
 *
 * When the app state is `app.users` (or any child state),
 * and contains the state parameter "users" with value "bilbobaggins",
